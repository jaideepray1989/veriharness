...
```text
self-accept: full trace + leaf self-acceptance
H1: summary + self accept
H2: state context + self accept
gated-resample: state context + external gates + feedback-free resampling
typed-repair+retain: gated VeriHarness repair + candidate retention
```

Older configs and result files may use `H0`, `H3`, and `H4`. They remain
accepted as aliases for `self-accept`, `gated-resample`, and
`typed-repair+retain`, respectively.
