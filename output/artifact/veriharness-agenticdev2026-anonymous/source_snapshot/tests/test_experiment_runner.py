...
from veriharness.experiments.aggregate import aggregate_rows, prompt_token_overhead, read_results
...
def test_legacy_variant_labels_parse_to_descriptive_names():
    config = ExperimentConfig(
        experiment_id="legacy-labels",
        variants=["H0", "H3", "H4"],
    )

    assert [variant.value for variant in config.variants] == [
        "self-accept",
        "gated-resample",
        "typed-repair+retain",
    ]
...
def test_prompt_overhead_normalizes_legacy_artifact_paths(tmp_path):
    transcript = tmp_path / "artifacts/leaves/H4/task/attempt_1/candidate-0/transcript.txt"
    transcript.parent.mkdir(parents=True)
    transcript.write_text("one two three", encoding="utf-8")

    overhead = prompt_token_overhead(tmp_path)

    assert overhead["by_variant"]["typed-repair+retain"]["retry_leaf_count"] == 1
...
def test_smoke_experiment_produces_results(tmp_path):
    config = ExperimentConfig(
        ...
        variants=[HarnessVariant.SELF_ACCEPT, HarnessVariant.GATED_RESAMPLE, HarnessVariant.TYPED_REPAIR_RETAIN],
    ...
    assert any(row["premature_stop"] for row in rows if row["variant"] == "self-accept")
...
def test_resume_skips_completed_rows_and_finishes_run(tmp_path):
    config = ExperimentConfig(
        ...
        variants=[HarnessVariant.SELF_ACCEPT, HarnessVariant.GATED_RESAMPLE],
...
def test_resume_distinguishes_reused_task_ids_across_seeds(tmp_path):
    ...
    run_dir = orch.run()
    rows = read_results(run_dir)
    kept = [row for row in rows if row["variant"] == "self-accept"]
    assert len(kept) == 4
...
def test_h3_rejects_invalid_output_even_if_done(tmp_path):
    ...
    config = ExperimentConfig(
        ...
        benchmarks=[],
        variants=[HarnessVariant.GATED_RESAMPLE],
        budget=BudgetConfig(max_retries=0),
    )
    ...
    result = orch.run_task_variant(task, HarnessVariant.GATED_RESAMPLE, store, event_log, state)
...
def test_h4_selects_passing_veriharness_candidate(tmp_path):
    ...
    config = ExperimentConfig(
        ...
        benchmarks=[],
        variants=[HarnessVariant.TYPED_REPAIR_RETAIN],
    ...
    result = orch.run_task_variant(task, HarnessVariant.TYPED_REPAIR_RETAIN, store, event_log, state)
...
def test_best_of_n_gated_spends_budget_on_candidates_without_repair(tmp_path):
    task = generate_context_trace_tasks(n_tasks=1, seed=1)[0]
    config = ExperimentConfig(
        experiment_id="best-of-n-gated-test",
        benchmarks=[],
        variants=[HarnessVariant.BEST_OF_N_GATED],
        budget=BudgetConfig(max_retries=3, veriharness_k=2, max_leaf_calls_per_task=3),
    )
    bad = {
        "task_id": task.task_id,
        "answer": '{"export_format": "csv", "fields": ["id", "value"]}',
    ...
    orch = Orchestrator(config, client=client, run_manager=RunManager(tmp_path))
    _, _, store, event_log, state = orch.run_manager.create(config, config.model_dump())
    result = orch.run_task_variant(task, HarnessVariant.BEST_OF_N_GATED, store, event_log, state)

    assert result.success
    assert result.num_leaf_calls == 3
    assert result.num_retries == 0
    assert result.metadata["candidate_count"] == 3
    assert result.metadata["candidate_retention"] is False
    assert result.metadata["repair_policy"] == "none_best_of_n"
    assert result.metadata["baseline_family"] == "best_of_n_gated"


def test_langgraph_validator_retry_uses_graph_feedback_without_typed_payload(tmp_path):
    task = generate_boolq_tasks(n_tasks=1, seed=1)[0]
    correct_answer = bool(task.hidden_oracle_payload["answer"])
    config = ExperimentConfig(
        experiment_id="langgraph-validator-retry-test",
        benchmarks=[],
        variants=[HarnessVariant.LANGGRAPH_VALIDATOR_RETRY],
        budget=BudgetConfig(max_retries=2, max_leaf_calls_per_task=2),
        evaluation={"oracle_guided_acceptance": False},
    )
    missing_artifact = {
    ...
    orch = Orchestrator(config, client=client, run_manager=RunManager(tmp_path))
    _, run_dir, store, event_log, state = orch.run_manager.create(config, config.model_dump())
    result = orch.run_task_variant(task, HarnessVariant.LANGGRAPH_VALIDATOR_RETRY, store, event_log, state)

    assert result.success
    assert result.metadata["repair_policy"] == "langgraph_validator_retry"
    assert result.metadata["repair_target_policy"] == "generate_validate_retry_graph"
    assert result.metadata["baseline_family"] == "validator_retry_graph"
    retry_context = (run_dir / result.run_path / "context_pack.md").read_text()
    assert "Validator-retry graph state" in retry_context
    assert "The artifact check reported" in retry_context
    assert "artifact_missing" not in retry_context
    assert "artifact.artifact_missing" not in retry_context
...
def test_aggregation_and_plots(tmp_path):
    ...
    assert aggregate["context_overload_slope"]["self-accept"] == 1.0
    assert aggregate["failure_modes_by_benchmark"]["context_trace"]["self-accept"]["context_bloat_proxy_rate"] == 0.5
    assert aggregate["failure_modes_by_benchmark"]["context_trace"]["self-accept"]["self_biased_acceptance_rate"] == 0.5
...
def test_compile_workshop_bundle_writes_tables(tmp_path):
    ...
    run_dir.mkdir()
    rows = []
    for variant, success in [("self-accept", False), ("gated-resample", True), ("typed-repair+retain", True)]:
        rows.append({
            "experiment_id": "e",
            "task_id": "task-1",
            "benchmark": "context_trace",
            ...
            "trace_length": 4,
            "success": success,
            "accepted_by_agent": variant == "self-accept",
            "accepted_by_gate": variant != "self-accept",
            "premature_stop": variant == "self-accept",
            "wrong_claim_accepted": False,
            "constraint_violation": variant == "self-accept",
            "tokens_in": 10,
            "tokens_out": 2,
            "num_leaf_calls": 1,
            "num_retries": 0,
            "wall_time_sec": 0.1,
            "failure_reasons": ["constraint_forgotten"] if variant == "self-accept" else [],
