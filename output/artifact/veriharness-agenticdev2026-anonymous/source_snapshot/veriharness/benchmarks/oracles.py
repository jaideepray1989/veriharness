...
from veriharness.benchmarks.textworld import evaluate_textworld_plan
...
def evaluate_oracle(task: TaskSpec, output: LeafOutput) -> GateResult:
    ...
    if oracle_type == "textworld":
        return textworld_oracle(task, output)
...
def textworld_oracle(task: TaskSpec, output: LeafOutput) -> GateResult:
    data = _answer_dict(output)
        ...
        return GateResult(
            ...
            failures=[
                GateFailure(
                    ...
                    message="The answer must be JSON with a commands list of strings.",
                    details={
    ...
    try:
        result = evaluate_textworld_plan(task, commands)
    except Exception as exc:
        return GateResult(
            gate_name="oracle",
            passed=False,
            score=0.0,
            failures=[GateFailure(code="environment_error", message=str(exc))],
        )
    if result.get("passed"):
        return GateResult(
            ...
            score=1.0,
            metadata={
                "turns_executed": result.get("turns_executed"),
    ...
    return GateResult(
        ...
        failures=[
            GateFailure(
                code=str(failure.get("code", "environment_error")),
                message=str(failure.get("message", "TextWorld evaluation failed.")),
                details=dict(failure.get("details", {})),
            )
        ],
        metadata={"trace": failure.get("details", {}).get("trace", [])},
    )
