...
from veriharness.benchmarks.humaneval import generate_humaneval_public_tasks, generate_humaneval_tasks
from veriharness.benchmarks.mini_workflow import (
    generate_mini_workflow_heldout_tasks,
    generate_mini_workflow_tasks,
)
...
def generate_named_tasks(
    ...
    if benchmark in {"mini_workflow_heldout", "mini-workflow-heldout"}:
        return generate_mini_workflow_heldout_tasks(n_tasks=n_tasks, seed=seed)
    if benchmark in {"textworld", "text-world"}:
        return generate_textworld_tasks(n_tasks=n_tasks, seed=seed)
    if benchmark in {"humaneval", "human-eval"}:
        return generate_humaneval_tasks(n_tasks=n_tasks, seed=seed)
    if benchmark in {"humaneval_public", "humaneval-public", "human-eval-public"}:
        return generate_humaneval_public_tasks(n_tasks=n_tasks, seed=seed)
