...
def render_leaf_prompt(request: LeafRequest) -> str:
    ...
    claims_guidance = (
        "This task does not require evidence claims: return an empty claims array unless a claim is necessary."
        if not request.task.hidden_oracle_payload.get("require_evidence", True)
        else "List claims explicitly with evidence references."
    )
...
- List "workflow_patch.txt" in artifacts."""
    if family in {"humaneval", "humaneval_public"}:
...
- Use claims: [] and self_assessment: {{}} unless a concrete risk must be recorded.
...
Visible public test:
{request.task.input_payload.get('public_test', 'No public test is exposed for this lane.')}"""
...
- List "solution.py" in artifacts."""
    if family == "textworld":
        return """For this textworld task:
- Solve the visible objective using a compact sequence of text-game commands.
- The LeafOutput.answer field must itself be JSON like {"commands":["go north","open chest"]}.
- Use at most the command budget in the context pack.
- Commands are evaluated from the initial game state in order.
- Do not include markdown fences inside the answer field.
...
def _attempt_guidance(request: LeafRequest) -> str:
    ...
    if request.task.family == "text_classification":
        ...
        return _strategy_guidance(request, strategies[candidate_index % len(strategies)])
    if request.task.family == "textworld":
        strategies = [
            "Follow the objective in its stated order and produce only the commands needed for those steps.",
            "Use the visible room description to identify the first movement or object interaction required by the objective.",
            "Prefer a minimal goal-directed plan; avoid look, inventory, and examine commands unless the objective requires them.",
            "Check that each planned command is plausible from the initial room before committing the bounded sequence.",
        ]
        return _strategy_guidance(request, strategies[candidate_index % len(strategies)])
    if request.task.family not in {"humaneval", "humaneval_public"}:
        return ""
    strategies = [
