...
def build_context_pack(
    ...
    if task.family == "context_trace":
        ...
        if task.family == "humaneval_public":
            current_state["visible_public_test"] = payload.get("public_test", "")
            constraints.append("Use the visible public test; hidden tests are unavailable during repair.")
        if variant == HarnessVariant.SELF_ACCEPT:
        ...
        current_state.update(
            {
                ...
                "initial_description": payload.get("description", ""),
                "initial_inventory": payload.get("inventory", ""),
                "max_steps": payload.get("max_steps", 4),
            }
        )
        constraints.extend(
            [
                "Return a JSON command plan in the LeafOutput.answer field.",
                "Use no more than the stated command budget.",
                "List action_plan.json in artifacts.",
            ]
        )
        accepted.append("The TextWorld environment validates command admissibility and terminal quest completion.")
