...
def build_retry_feedback(
    ...
    if task.family == "context_trace":
    ...
    elif task.family in {"humaneval", "humaneval_public"}:
        ...
        add("The answer field must contain Python source code that assigns result.")
        add('List "solution.py" in artifacts so the candidate can be checked.')
        if "result_assignment_missing" in failure_codes:
            add("Assign the final value to a variable named result.")
        if "syntax_error" in failure_codes:
            add("Fix Python syntax and indentation before changing the solution logic.")
    elif task.family == "textworld":
        max_steps = int(task.input_payload.get("max_steps", 4))
        add('The answer field must be JSON like {"commands":["go north","open chest"]}.')
        add(f"Use no more than {max_steps} commands.")
        add('List "action_plan.json" in artifacts.')
        if {"action_invalid", "command_missing", "action_plan_invalid"} & failure_codes:
            add("Replace the rejected command at the typed location with an admissible command from the current state.")
        if {"quest_incomplete", "game_lost", "turn_budget_exceeded"} & failure_codes:
            add("Revise the command sequence to complete the visible objective within the command budget.")
...
def build_same_info_natural_feedback(
    task: TaskSpec,
    _context: ContextPack,
    output: LeafOutput,
    gate_results: List[GateResult],
) -> List[str]:
    """Render the typed-fields payload as prose without typed labels or keys."""
    feedback: List[str] = []
    ...
    add("The previous attempt failed validation. Return one complete corrected LeafOutput JSON object.")
    for result in gate_results:
        for failure in result.failures:
            record = _typed_failure_record(task, output, result, failure)
            add(
                f"The rejected value was at {record['location']}. "
                f"The validator observed {record['observed']}. "
                f"It expected {record['expected']}."
            )
    if output.answer:
...
def build_location_observed_feedback(
    task: TaskSpec,
    _context: ContextPack,
    output: LeafOutput,
    gate_results: List[GateResult],
) -> List[str]:
    """Expose typed label, location, and observation but no alternatives."""
    feedback: List[str] = []
...
def build_targeted_untyped_feedback(
    ...
    elif {"entry_point_missing", "code_missing", "result_assignment_missing", "syntax_error", "timeout", "runtime_error", "unit_test_failed"} & target_codes:
        feedback.extend(_code_target_guidance(task, target_codes))
    elif {"action_plan_invalid", "command_missing", "turn_budget_exceeded", "action_invalid", "quest_incomplete", "game_lost"} & target_codes:
        feedback.extend(_textworld_target_guidance(task, target_codes))
    elif {
        "expected_substring_missing",
        "forbidden_substring_present",
        "json_field_mismatch",
...
def required_artifact_for(task: TaskSpec) -> str:
    ...
    if task.family in {"humaneval", "humaneval_public"}:
        return "solution.py"
    if task.family == "ds1000":
        return "solution.py"
    if task.family == "textworld":
        return "action_plan.json"
...
def _failure_location(
    ...
    if "location" in details:
        return _short(details["location"])
    ...
    if code in {"entry_point_missing", "code_missing", "result_assignment_missing", "syntax_error", "timeout", "runtime_error", "unit_test_failed"}:
        entry_point = str(task.input_payload.get("entry_point", ""))
...
def _failure_observed(output: LeafOutput, code: str, details: Dict[str, Any]) -> str:
    for key in ["observed", "actual", "action", "answer", "stdout", "stderr"]:
        if key in details:
...
def _preserve_set_instructions(task: TaskSpec, output: LeafOutput, gate_results: List[GateResult]) -> List[str]:
    ...
    if task.family == "mini_workflow":
        ...
        instructions.append("Preserve-set: keep valid command prefixes and replace only the typed failing command or suffix.")
    return instructions
...
def _code_target_guidance(task: TaskSpec, target_codes: set[str]) -> List[str]:
    entry_point = str(task.input_payload.get("entry_point", ""))
    guidance = ["Focus first on returning executable Python source code rather than prose."]
    if task.family == "ds1000":
        guidance.append("The code must assign the final value to a variable named result.")
    else:
        guidance.append(f"The code must define the requested entry point: {entry_point}.")
    guidance.append('List "solution.py" in artifacts.')
...
def _textworld_target_guidance(task: TaskSpec, target_codes: set[str]) -> List[str]:
    max_steps = int(task.input_payload.get("max_steps", 4))
    guidance = [
        "Return a JSON command plan, not prose.",
        f"Keep the plan to at most {max_steps} commands.",
        'List "action_plan.json" in artifacts.',
    ]
    if {"action_invalid", "command_missing", "action_plan_invalid"} & target_codes:
        guidance.append("Use the typed expected commands and location to replace only the rejected action.")
    if {"quest_incomplete", "game_lost", "turn_budget_exceeded"} & target_codes:
        guidance.append("Keep any valid prefix and revise the remaining actions to finish the visible objective.")
    return guidance
