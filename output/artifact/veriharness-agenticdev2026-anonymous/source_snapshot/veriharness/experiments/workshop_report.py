...
def _baseline_comparison(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for model in by_model:
        ...
        for treatment in ["gated-resample", "typed-repair+retain"]:
            baseline = [row for row in model_rows if row.get("variant") == "self-accept"]
...
def _failure_examples(run_dirs: List[Path]) -> List[Dict[str, Any]]:
    ...
    selectors: List[tuple[str, Callable[[Dict[str, Any]], bool]]] = [
        ...
        (
            "gate_repair_success",
            lambda row: row.get("variant") == "typed-repair+retain"
...
def _baseline_markdown(rows: List[Dict[str, Any]]) -> str:
    lines = [
        "# Baseline Comparison",
        "",
        f"Baseline: `{BASELINE_NAME}`. Operationally this is `self-accept`: full trace in context, no independent acceptance gate, and leaf self-assessment controls done.",
        "",
        "| Axis | Baseline | `typed-repair+retain` (VeriHarness) |",
        "|---|---|---|",
...
def _baseline_takeaway(rows: List[Dict[str, Any]]) -> str:
    typed_repair = [row for row in rows if row["treatment"] == "typed-repair+retain"]
    if not typed_repair:
        return "No `typed-repair+retain` baseline comparison rows were available."
    base_success = sum(int(row["baseline_success"]) for row in typed_repair)
    base_n = sum(int(row["baseline_n"]) for row in typed_repair)
    treatment_success = sum(int(row["treatment_success"]) for row in typed_repair)
    treatment_n = sum(int(row["treatment_n"]) for row in typed_repair)
    base_premature = sum(int(row["baseline_premature"]) for row in typed_repair)
    treatment_premature = sum(int(row["treatment_premature"]) for row in typed_repair)
    return (
        f"Against {BASELINE_NAME}, `typed-repair+retain` solves {treatment_success}/{treatment_n} "
