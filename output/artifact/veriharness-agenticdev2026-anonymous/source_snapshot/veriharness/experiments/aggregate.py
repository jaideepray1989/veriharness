...
POLICY_TEST_PAIRS = [
    ("self-accept", "gated-resample"),
    ("gated-resample", "generic-retry"),
    ("gated-resample", "best-of-n-gated"),
    ("best-of-n-gated", "generic-retry"),
    ("generic-retry", "generic+diagnostics"),
    ("generic+diagnostics", "typed-label-only"),
    ("generic+diagnostics", "typed-fields"),
    ("generic+diagnostics", "same-info-natural"),
    ("same-info-natural", "typed-fields"),
    ("location-observed", "typed-fields"),
    ("typed-label-only", "typed-fields"),
    ("typed-fields", "typed-preserve"),
    ("typed-fields", "typed-repair+retain"),
    ("generic+diagnostics", "typed-repair+retain"),
    ("generic-retry", "typed-preserve"),
    ("generic-retry", "natural-retry"),
    ("natural-retry", "langgraph-validator-retry"),
    ("generic-retry", "retain+generic"),
    ("natural-retry", "targeted+untyped"),
    ("targeted+untyped", "typed+no-retain"),
    ("typed+no-retain", "typed-repair+retain"),
    ("retain+generic", "typed-repair+retain"),
    ("generic-retry", "typed-repair+retain"),
    ("gated-resample", "typed-repair+retain"),
]
...
def read_results(run_dir: Path) -> List[Dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            if line.strip():
                row = json.loads(line)
                row["variant"] = canonical_variant_name(row.get("variant", ""))
                rows.append(row)
    return rows
...
def prompt_token_overhead(run_dir: Path) -> Dict[str, Any]:
    ...
    for transcript in leaves_dir.rglob("transcript.txt"):
        ...
        if not parts:
            continue
        variant = canonical_variant_name(parts[0])
        attempt = next(
            (
                part
                for part in parts
