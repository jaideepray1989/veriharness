...
from veriharness.experiments.typed_repair_report import compile_typed_repair_evidence
...
def export_swebench_predictions_cmd(
    ...
    variant: str = typer.Option(..., help="Variant to export, e.g. typed-repair+retain."),
...
@app.command("compile-typed-repair-evidence")
def compile_typed_repair_evidence_cmd(
    run_dirs: str = typer.Option(..., help="Comma-separated run directories."),
    out: Path = typer.Option(Path("reports/cross_benchmark_typed_repair_evidence.md"), help="Report path."),
    expected_rows_per_model: int = typer.Option(1992, help="Complete row count for one model."),
) -> None:
    """Compile the cross-benchmark typed-repair paper evidence table."""
    paths = [_resolve_run_dir(Path(item.strip())) for item in run_dirs.split(",") if item.strip()]
    summary = compile_typed_repair_evidence(
        paths,
        out,
        expected_rows_per_model=expected_rows_per_model,
    )
    console.print(json.dumps(summary, indent=2, sort_keys=True))
