...
Structured verifier feedback more than doubles terminal success under equal
compute. On 50 paired TextWorld games with a four-call cap, it raises success
from 14/50 to 36/50 for Qwen2.5-Coder-14B (+44 points, 95\%
paired-bootstrap interval 28--60) and from 8/50 to 29/50 for Llama-3.1-8B
(+42 points, 28--56). The win is driven by repair content: exposing the failing
location, observed value, and admissible alternatives; omitting alternatives
largely removes it. The gain persists across call budgets and sampled decoding.
A small HumanEval scope check identifies the boundary: feedback cannot repair a
failure the visible validator does not expose. Across 880 rows and 2,652 LLM
calls, the evidence supports a practical design rule: treat validator failures
as structured repair interfaces, not retry messages.
\end{abstract}
...
\subsection{Repair Content, Not JSON Syntax, Explains the Gain}
